...
const { updateRepo, printRepoData } = require('./db_repos_data'); // Import the new function
...
async function processRepos() {
    while (true) {
        try {
            ...
            if (repos.length === 0) {
                console.log('No repos fetched from the queue. Sleeping for 60 minutes...');
            ...
            for (const repo of repos) {
                try {
                    ...
                    if (repoError.response && repoError.response.status === 429) {
